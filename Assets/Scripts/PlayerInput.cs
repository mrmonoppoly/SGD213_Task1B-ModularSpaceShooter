...
public class PlayerInput : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (horizontalInput != 0.0f)
        {
            ...
            if (EngineBase != null)
            {
                ...
                EngineBase.MovePlayer(horizontalInput * Vector2.right);
            }
        }
    ...
    public void SwapWeapon(WeaponType weaponType)
    {
        ...
        if (weaponType == WeaponType.tripleShot) {
            ...
            tripleShotCoroutine = StartCoroutine(ExpireWeapon(WeaponType.machineGun, 10f));        
        }
    }

    private Coroutine tripleShotCoroutine;
    
    private IEnumerator ExpireWeapon(WeaponType fallbackType, float duration) {
        yield return new WaitForSeconds(duration);
        SwapWeapon(fallbackType);
